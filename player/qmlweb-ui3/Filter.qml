// Предназначение - создает функцию для фильтрации объектов в списке объектов
// затем она используется в UI и передается в Shower

// @idea не regexp а xpath (аля css)
// @idea fuzzy поиск если ошибся то и ладно примерно хотьнайдем
// @idea транслит раскладки клавы см ниже
...
TextField {
      ...
      // оказалось что очистка по ентеру это неожиданное поведение для интуиции (моей)
      // по ентеру хочется перейти в список объектов
